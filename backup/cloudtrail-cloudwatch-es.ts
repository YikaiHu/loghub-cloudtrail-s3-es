...
function transform(payload: any) {
    ...
    payload.logEvents.forEach(function (logEvent: any) {
...
function buildSource(message: any, extractedFields: any) {
    if (extractedFields) {
        var source = {};
        interface ISource {
            [key: string]: any
        ...
        for (var key in extractedFields) {
            if (extractedFields.hasOwnProperty(key) && extractedFields[key]) {
                ...
                if (isNumeric(value)) {
                    (<ISource>source)[key] = 1 * value;
                    continue;
                }

                var jsonSubString = extractJson(value);
                if (jsonSubString !== null) {
                    (<ISource>source)['$' + key] = JSON.parse(jsonSubString);
                }

                (<ISource>source)[key] = value;
            }
        }
        return source;
    }
